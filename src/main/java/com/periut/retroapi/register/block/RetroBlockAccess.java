...
public interface RetroBlockAccess {
	 ...
	 *
	 * <p>Works under StationAPI too, by a different route. Arsenic {@code @Overwrite}s the vanilla method
	 * this reads and delegates to a renderer of its own, so the constant is simply not there any more, but
	 * it kept beta's rule verbatim one class along in {@code ArsenicItemRenderer.renderVanilla}. RetroAPI's
	 * StationAPI module corrects it there; nothing about the declaration changes either way.
	 *
