...
public class WorldConversionProcessor {
	 ...
	 */
	private static void carryOverXmeta(NbtCompound oldSidecar, int chunkX, int chunkZ, NbtCompound chunkNbt) {
		if (oldSidecar == null || !oldSidecar.contains("chunks")) return;
		NbtCompound chunks = oldSidecar.getCompound("chunks");
		String key = chunkX + "," + chunkZ;
		if (!chunks.contains(key)) return;
		NbtCompound old = chunks.getCompound(key);
		byte[] xpos = old.getByteArray("xpos");
		byte[] xval = old.getByteArray("xval");
		if (xpos.length > 0 && xval.length * 4 == xpos.length) {
			chunkNbt.putByteArray("xpos", xpos);
			chunkNbt.putByteArray("xval", xval);
		}
	}
	...
	private static boolean injectInventorySidecar(NbtCompound level, int chunkX, int chunkZ,
		...
		if (chunkData.contains("inventoryItems")) {
			...
			for (int i = 0; i < inventoryItems.size(); i++) {
				...
				if (entry.contains("retroapi:components")) {
					item.put("retroapi:components", entry.getCompound("retroapi:components"));
				}
				...
				if (targetTE != null && targetTE.contains("Items")) {
					targetTE.getList("Items").add(item);
				} else {
		...
		if (chunkData.contains("fullEntities")) {
			...
			for (int i = 0; i < fullEntities.size(); i++) {
				NbtCompound entity = (NbtCompound) fullEntities.get(i);
			...
			level.put("Entities", entities);
			modified = true;
			LOGGER.debug("Injected {} modded full entities into chunk {},{}", fullEntities.size(), chunkX, chunkZ);
		}
	 ...
	 */
	private static void fixCarriedItems(NbtCompound entity) {
		for (String key : new String[]{"Items", "Inventory", "Equipment", "ArmorItems", "HandItems"}) {
			if (entity.contains(key)) fixItemList(entity.getList(key));
		}
		if (entity.contains("Item")) fixSingleItem(entity.getCompound("Item"));
	}
	...
	private static void reconstructBlockSidecarsForDim(File worldDir, String regionRel, String sidecarNs) {
		...
		for (File regionFile : regionFiles) {
			...
			NbtCompound oldBlockSidecar = loadSidecarNbt(
				...
				for (int localX = 0; localX < 32; localX++) {
					for (int localZ = 0; localZ < 32; localZ++) {
						...
						if (!positions.isEmpty()) {
							...
							String chunkKey = chunkX + "," + chunkZ;
							sidecarChunks.put(chunkKey, chunkNbt);
							hasData = true;
						}
					}
				}
			...
			if (hasData) {
				sidecarRoot.put("chunks", sidecarChunks);
	...
	private static void reconstructInventorySidecarsForDim(File worldDir, String regionRel, String sidecarNs) {
		...
		for (File regionFile : regionFiles) {
				...
				for (int localX = 0; localX < 32; localX++) {
					for (int localZ = 0; localZ < 32; localZ++) {
						...
						if (level.contains("Entities")) {
							...
							NbtList moddedFullEntities = new NbtList();
							...
							for (int i = 0; i < entities.size(); i++) {
								...
								if ("Item".equals(entityId)) {
									...
									moddedFullEntities.add(entity);
									continue;
								}
								filteredEntities.add(entity);
							}

							if (moddedItemEntities.size() > 0 || moddedFullEntities.size() > 0) {
								level.put("Entities", filteredEntities);
								if (moddedItemEntities.size() > 0) chunkData.put("itemEntities", moddedItemEntities);
								if (moddedFullEntities.size() > 0) chunkData.put("fullEntities", moddedFullEntities);
								chunkModified = true;
							}
						}

						if (chunkModified) {
							invSidecarChunks.put(chunkKey, chunkData);
							hasData = true;
			...
			if (hasData) {
				invSidecarRoot.put("chunks", invSidecarChunks);
				File invSidecarFile = new File(worldDir, sidecarNs + "/inventories/r." + regionX + "." + regionZ + ".dat");
	...
	private static void tagCarriedItems(NbtCompound entity, Map<Integer, String> reverseBlockMap,
										Map<Integer, String> reverseItemMap) {
		for (String key : new String[]{"Items", "Inventory", "Equipment", "ArmorItems", "HandItems"}) {
			if (entity.contains(key)) tagItemsWithRetroId(entity.getList(key), reverseBlockMap, reverseItemMap);
		}
		if (entity.contains("Item")) {
			NbtCompound item = entity.getCompound("Item");
			String retroId = resolveRetroId(item, reverseBlockMap, reverseItemMap);
	 ...
	 */
	private static String[] decodeSidecarIds(NbtCompound chunkNbt, int posCount) {
