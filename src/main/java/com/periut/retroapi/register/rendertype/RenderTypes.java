...
public final class RenderTypes {
	...
	/**
	 * The built-in JSON-model render type. Blocks with a blockstate JSON are wired to it
	 * automatically at registration; set it explicitly with {@code .renderType(RenderTypes.MODEL)}
	 * when supplying the table from code. The renderer itself attaches client-side.
	 */
	public static final NamespacedIdentifier MODEL = NamespacedIdentifiers.from("retroapi", "model");
