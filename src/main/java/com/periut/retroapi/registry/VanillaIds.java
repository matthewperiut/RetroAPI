...
/**
 * Identifier names for all vanilla b1.7.3 blocks and items, copied 1:1 from StationAPI's
 * {@code StationFlatteningItemStackSchema} (station-vanilla-fix-v0) - the same names StationAPI
 * uses in flattened world saves, so identifiers stay interchangeable between the two APIs and a
 * future RetroAPI datafixer can reuse this table verbatim.
 *
 * <p>{@code putState} mirrors the schema: it registers the name as BOTH a block and an item
 * (block-items share the block id). {@code putItem} registers item-only names. Where vanilla has
 * a block and a distinct inventory item under the same name (oak_door, red_bed, cake, repeater,
 * oak_sign, wheat, sugar_cane, iron_door), the LATER putItem wins the name->item-id mapping,
 * exactly like the schema's overwrite behavior - so "minecraft:oak_door" resolves to the door
 * ITEM (324) while blockId("minecraft:oak_door") still resolves to the door BLOCK (64).
 */
public final class VanillaIds {
	private static final Map<Integer, String> BLOCK_ID_TO_NAME = new LinkedHashMap<>();
	private static final Map<String, Integer> BLOCK_NAME_TO_ID = new LinkedHashMap<>();
	private static final Map<Integer, String> ITEM_ID_TO_NAME = new LinkedHashMap<>();
	private static final Map<String, Integer> ITEM_NAME_TO_ID = new LinkedHashMap<>();

	private VanillaIds() {
	}

	private static void putState(int id, String name) {
		BLOCK_ID_TO_NAME.put(id, name);
		BLOCK_NAME_TO_ID.put(name, id);
		putItem(id, name);
	}

	private static void putItem(int id, String name) {
		ITEM_ID_TO_NAME.put(id, name);
		ITEM_NAME_TO_ID.put(name, id);
	}

	/** Vanilla block identifier ("minecraft:...") for a numeric block id, or null. */
	public static String blockName(int blockId) {
		return BLOCK_ID_TO_NAME.get(blockId);
	}

	/** Numeric block id for a vanilla block identifier, or -1. */
	public static int blockId(String identifier) {
		Integer id = BLOCK_NAME_TO_ID.get(identifier);
		return id == null ? -1 : id;
	}

	/** Vanilla item identifier ("minecraft:...") for a numeric item id, or null. */
	public static String itemName(int itemId) {
		return ITEM_ID_TO_NAME.get(itemId);
	}

	/** Numeric item id for a vanilla item identifier, or -1. */
	public static int itemId(String identifier) {
		Integer id = ITEM_NAME_TO_ID.get(identifier);
		return id == null ? -1 : id;
	}

	/** All vanilla item identifiers (includes block-items), insertion-ordered. */
	public static Collection<String> itemIdentifiers() {
		return Collections.unmodifiableCollection(ITEM_NAME_TO_ID.keySet());
	}

	/** All vanilla block identifiers, insertion-ordered. */
	public static Collection<String> blockIdentifiers() {
		return Collections.unmodifiableCollection(BLOCK_NAME_TO_ID.keySet());
	}
